//You can only include the header file for the Ticket because <iostream>, <cstring> and Person.h are already included there and they will be read here 
#include "Airplane.h"

void fillTicketSchedule(int quantity, AirplaneTicket* flightSchedule) {
	for (int i = 0; i < quantity; i++) {
		flightSchedule[i].fillTicketInformation();
	}
}

void printTicketScheduleByStartingPoint(int quantity, AirplaneTicket* flightSchedule, const char* startingPoint) {
	for (int i = 0; i < quantity; i++) {
...
void printTicketScheduleByDestAndDate(int quantity, AirplaneTicket* flightSchedule, const char* destination, const char* date) {
	for (int i = 0; i < quantity; i++) {
		if (strcmp(flightSchedule[i].getStartingPoint(), destination) == 0 && strcmp(flightSchedule[i].getDate(), date) == 0) {
			flightSchedule[i].printTicket();
		}
	}
}

void printArrivalsByDate(int quantity, AirplaneTicket* flightSchedule, const char* date) {
	int count = 0;
	for (int i = 0; i < quantity; i++) {
...
bool compareFlights(const AirplaneTicket* first, const AirplaneTicket* second)
{
	return (strcmp(first->getStartingPoint(), second->getStartingPoint()) == 0 &&
		strcmp(first->getEndPoint(), second->getEndPoint()) == 0 &&
		strcmp(first->getDate(), second->getDate()) == 0);
}

bool isNotPrinted(int differentFlightTickets, const AirplaneTicket* currentFlight, const AirplaneTicket* alreadyPrinted) {
...
double calculateAveragePrice(int quantity, const AirplaneTicket* currentFlight, const AirplaneTicket* flightSchedule) {
	double sum = 0;
	int sameFlight = 0;
	for (int i = 0; i < quantity; i++) {
...
void printAveragePriceForFlight(int quantity, AirplaneTicket* flightSchedule) {
	...
	for (int uncheckedFlights = 0; uncheckedFlights < quantity; uncheckedFlights++) {
		if (isNotPrinted(differentFlightTickets, &flightSchedule[uncheckedFlights], alreadyPrinted)) {
			std::cout << "For flight: " <<
				...
				flightSchedule[uncheckedFlights].getEndPoint() << ", " <<
...
int countChildren(int quantity, const AirplaneTicket* currentFlight, const AirplaneTicket* flightSchedule) {
	int count = 0;
	for (int i = 0; i < quantity; i++) {
		if (compareFlights(currentFlight, &flightSchedule[i]) && flightSchedule[i].getHolderAge() <= 12) {
			count++;
		}
	}
	return count;
...
void printNumberOfChildren(int quantity, AirplaneTicket* flightSchedule) {
...
int countPeopleByFirstNameAndDate(int quantity, const AirplaneTicket* flightSchedule, const char* firstName, const char* date) {
	int count = 0;
	for (int i = 0; i < quantity; i++) {
...
int main() {
	...
	printTicketScheduleByDestAndDate(quantity, flightSchedule, "Milano", "09/05/2022");

	printArrivalsByDate(quantity, flightSchedule, "09/05/2022");
	...
	std::cout << countPeopleByFirstNameAndDate(quantity, flightSchedule, "Antonio", "09/05/2022");
